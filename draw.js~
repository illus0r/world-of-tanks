...
d3.csv("data.csv", function(error, csv) {
  if (error) return console.warn(error);
    d3.xml("img/svg/Рейдер.obj0001.svg", "image/svg+xml", function(xml) {
    var tank_svg_default = document.importNode(xml.documentElement, true);

    function load_svg( model_name ){
      jQuery('body').append('<div class="loader"></div>');
      jQuery('body .loader').load("img/svg/"+model_name+".obj0001.svg", null, function() { 
        jQuery('#theElement').click( function() {
          alert('You clicked on the element!');
        });
      });
      console.log("img/svg/"+model_name+".obj0001.svg");
      var tank_svg;
      d3.xml("img/svg/"+model_name+".obj0001.svg", "image/svg+xml", function(xml) {
        tank_svg = document.importNode(xml.documentElement, true);
      });
  ...
  svg.append("g")
    ...
    function update_scatterplot(){
      ...
      tanks_enter
        ...
        .each(function(d, i){ 
          var plane = this.appendChild(d.tank_svg.cloneNode(true)); 
